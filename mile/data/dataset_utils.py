...
def pkl_preprocess(data, forward_step):
    # forward_step = 6
    location_data = data['location_actor']
    length = len(location_data)
    next_point_location = []
    for i in range(length):
        next_step = i + forward_step
        if next_step >= length:
            next_step = length - 1
        dx = location_data[next_step][0][0] - location_data[i][0][0]
        dy = location_data[next_step][0][1] - location_data[i][0][1]
        dz = location_data[next_step][0][2] - location_data[i][0][2]
        next_point_location.append(np.array([dx, dy, dz], dtype=np.float32))
    data['future_location'] = next_point_location
    return data

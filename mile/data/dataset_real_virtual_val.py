...
class DataModule(pl.LightningDataModule):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.val_dataset = CombineDataModule(self.cfg)
        self.val_dataset.setup()
        # self.train_dataset = CombineDataModule(self.cfg)
        # self.val_dataset = None
        self.batch_size = self.cfg.BATCHSIZE
        self.sequence_length = self.cfg.RECEPTIVE_FIELD + self.cfg.FUTURE_HORIZON
        print(self.cfg.RECEPTIVE_FIELD,self.sequence_length)
...
class CombineDataModule(Dataset):
    ...
    def setup(self, stage=None):
        self.train_carla = CarlaDataset(self.cfg, mode='val', sequence_length=self.sequence_length)
        # self.val_carla = CarlaDataset(self.cfg, mode='val', sequence_length=self.sequence_length)

        self.train_nusc = FuturePredictionDataset(nusc=self.nusc, is_train=1, cfg=self.cfg)
        ...
        for town_path in towns:
            ...
            for run_path in runs:
                ...
                if os.path.isfile(pd_dataframe_path):
                    self.data[f'{town}/{run}'] = pd.read_pickle(pd_dataframe_path).transpose() #因为过来的数据倒过来了，加个转置
                    self.data[f'{town}/{run}'] = pkl_preprocess(self.data[f'{town}/{run}'], self.cfg.FUTURE_HORIZON)

...
class MsImageDis(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.n_layer = self.cfg.DIS.N_LAYER
        self.gan_type = self.cfg.DIS.GAN_TYPE
        self.dim = 32 #64占显存较大
        self.norm ='none'
        self.activ = 'lrelu'
        self.num_scales = 3
    ...
    def forward(self, x):
        outputs = []
        #log.info(f'dis input.shape is {x.shape}')
        for model in self.cnns:
            outputs.append(model(x))
            #log.info(f'output.shape is{model(x).shape}')
            x = self.downsample(x)
            #log.info(f'x.shape is{x.shape}')
        #log.info(f'dis toatal out.size is {len(outputs)}')
        return outputs

    def calc_dis_loss(self, input_fake, input_real):
        # calculate the loss to train D
        input_fake = pack_sequence_dim(input_fake)
        input_real = pack_sequence_dim(input_real)

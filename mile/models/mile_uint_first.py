...
class WorldGen(nn.Module):
    def __init__(self, cfg):
        ...
        if self.cfg.MODEL.ENCODER.NAME == 'resnet18':
            self.encoder_nusc = timm.create_model(
                cfg.MODEL.ENCODER.NAME, pretrained=True, features_only=True, out_indices=[2, 3, 4],
            )
            feature_info_nusc = self.encoder_nusc.feature_info.get_dicts(keys=['num_chs', 'reduction'])

        self.feat_decoder_nusc = Decoder(feature_info_nusc, self.cfg.MODEL.ENCODER.OUT_CHANNELS)
        ...
        if not self.cfg.EVAL.NO_LIFTING:
            ...
            self.frustum_pooling_nusc = FrustumPooling(
                size=(cfg.BEV.SIZE[0] // bev_downsample, cfg.BEV.SIZE[1] // bev_downsample),
                scale=cfg.BEV.RESOLUTION * bev_downsample,
                offsetx=cfg.BEV.OFFSET_FORWARD / bev_downsample,
                dbound=cfg.BEV.FRUSTUM_POOL.D_BOUND,
            ...
            self.depth_decoder_nusc = Decoder(feature_info, self.cfg.MODEL.ENCODER.OUT_CHANNELS)
            self.depth_nusc = nn.Conv2d(self.depth_decoder.out_channels, self.frustum_pooling.D, kernel_size=1)
        ...
        self.backbone_bev_nusc = timm.create_model(
            cfg.MODEL.BEV.BACKBONE,
            in_chans=backbone_bev_in_channels,
            pretrained=True,
            features_only=True,
        ...
        feature_info_bev_nusc = self.backbone_bev_nusc.feature_info.get_dicts(keys=['num_chs', 'reduction'])
        ...
        self.final_state_conv_nusc = nn.Sequential(
            BasicBlock(feature_info_bev[-1]['num_chs'], embedding_n_channels, stride=2, downsample=True),
            BasicBlock(embedding_n_channels, embedding_n_channels),
            nn.AdaptiveAvgPool2d(output_size=(1, 1)),
            nn.Flatten(start_dim=1),
        ...
        self.posterior_carla = RepresentationModel(
            in_channels=embedding_n_channels,
            latent_dim=self.cfg.MODEL.TRANSITION.STATE_DIM,
        )

        self.posterior_nusc = RepresentationModel(
            in_channels=embedding_n_channels,
            latent_dim=self.cfg.MODEL.TRANSITION.STATE_DIM,
        )

    def gen_encoder(self, batch):
    ...
    def forward(self, batch, deployment=False):
        ...
        b, s = batch_carla['image'].shape[:2]
        embedding_carla = self.encode(batch_carla)  # dim (b, s, 512)
        embedding_nusc = self.encode_nusc(batch_nusc)
        ...
        # else:
        #     state = embedding
        state_carla_mu, state_carla_sigma = self.posterior_carla(embedding_carla)
        state_nusc_mu, state_nusc_sigma = self.posterior_nusc(embedding_nusc)

        sample_carla = self.sample_from_distribution(state_carla_mu, state_carla_sigma, use_sample=True) 
        sample_nusc = self.sample_from_distribution(state_nusc_mu, state_nusc_sigma, use_sample=True)
    ...
    def encode_nusc(self, batch):
        ...
        # Image encoder, multiscale
        xs = self.encoder_nusc(image)

        # Lift features to bird's-eye view.
        # Aggregate features to output resolution (H/8, W/8)
        x = self.feat_decoder_nusc(xs)

        if not self.cfg.EVAL.NO_LIFTING:
            # Depth distribution
            depth = self.depth_nusc(self.depth_decoder_nusc(xs)).softmax(dim=1)
            ...
            x = self.frustum_pooling_nusc(x, intrinsics.unsqueeze(1), extrinsics.unsqueeze(1), depth_mask)

        embedding = self.backbone_bev_nusc(x)[-1]
        embedding = self.final_state_conv_nusc(embedding)
        embedding = unpack_sequence_dim(embedding, b, s)
        return embedding

...
class WorldGen(nn.Module):
    def __init__(self, cfg):
        ...
        if self.cfg.MODEL.ENCODER.NAME == 'resnet18':
            self.encoder = timm.create_model(
                cfg.MODEL.ENCODER.NAME, pretrained=True, features_only=True, out_indices=[4],
            )
        ...
        if self.cfg.EVAL.RGB_SUPERVISION:
            ...
            self.rgb_decoder = ResDecoder(n_upsample=3, n_res=4, dim=128, output_dim=3,)
    ...
    def encode(self, batch, img):
        ...
        embedding = unpack_sequence_dim(xs, b, s)
        return embedding

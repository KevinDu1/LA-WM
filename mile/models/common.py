...
class RGBHead(nn.Module):
    ...
    def forward(self, x):
        output = {
            f'rgb_label_{self.downsample_factor}': self.rgb_head(x),
        }
        return output
...
class RGBDecoder(nn.Module):
    def __init__(self, latent_n_channels, semantic_n_channels, constant_size=(3, 3), is_segmentation=True):
        ...
        self.middle_conv = nn.ModuleList(
            [DecoderBlock(n_channels, n_channels, latent_n_channels, upsample=True) for _ in range(2)]
        )
...
class Ncppolicy(nn.Module):
    def __init__(self, in_channels, fut):
        super().__init__()
        self.units = 128
        # self.latent_out = 64
        self.latent_out_ = 32
        self.rnn = CfC(in_channels, AutoNCP(self.units, self.latent_out_), batch_first = True)
        self.decoder_traj = CfC(self.latent_out_, AutoNCP(self.units, self.latent_out_), batch_first = True)
        self.fut = fut
        self.outlayer = nn.Linear(self.latent_out_, 2, bias=True)
        self.leaky_relu = torch.nn.LeakyReLU(0.01)
        
    def forward(self, x):
        h0 = torch.zeros(x.shape[0],self.units).to(x.device)
        dec_input = torch.zeros(x.shape[0], 1, self.latent_out_).to(x.device)
        _, hx = self.rnn(x,h0)
        fut_pred = torch.zeros(x.shape[0], self.fut, 2).to(x.device)  # 结果存储
        for t in range(self.fut):
            out, hx = self.decoder_traj(dec_input, hx)
            pred = self.leaky_relu(self.outlayer(out))
            dec_input = out
            fut_pred[:, t:t+1, :] = pred
        return fut_pred
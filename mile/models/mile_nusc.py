...
class Mile_UINT(nn.Module):
    ...
    def forward_dis(self, batch):
        ...
        h_b, noise = self.gen_b.gen_encoder(batch_b, batch_b['image'])
        # x_ba = self.gen_a.gen_decoder(h_b)
        x_recon = self.gen_b.gen_decoder(h_b + noise)
        #loss_dis
        losses_dis = dict()
        # losses_dis['dis_a_ba'] = self.cfg.DIS.GAN_WEIGHT * self.dis_a.calc_dis_loss(x_ba['rgb_label_1'].detach(), batch_a['image'])
        losses_dis['dis_b'] = self.cfg.DIS.GAN_WEIGHT * self.dis_b.calc_dis_loss(x_recon['rgb_label_1'].detach(), batch_b['image'])
    ...
    def forward_gen(self, batch):
        ...
        output = {}
        h_b, noise = self.gen_b.gen_encoder(batch_b, batch_b['image'])
        x_b_recon = self.gen_b.gen_decoder(h_b + noise)

        losses_gen = dict()
 
        losses_gen.update(self.cal_rgb_loss(x_b_recon, batch_b, 'x_b_recon')) #10
        losses_gen['loss_gen_adv_b'] = self.cfg.DIS.GAN_WEIGHT * self.dis_b.calc_gen_loss(x_b_recon['rgb_label_1']) #1

...
class PreProcess(nn.Module):
    ...
    def prepare_traj_labels(self, batch):
        traj = batch['gt_trajectory'][:,:,:2].clone()
        # relative_traj = traj - traj[:,:1,:]
        # batch['gt_trajectory'] = relative_traj[:,1:,:]
        relative_traj = traj - batch['now_t'][:,:,:2]
        batch['gt_trajectory'] = relative_traj.to(torch.float)
        batch['speed'] = batch['speed'].to(torch.float)
        batch['command'] = batch['command'].to(torch.float)
    ...
    def prepare_rgb_labels(self, batch):
    ...
    def forward(self, batch: Dict[str, torch.Tensor], ver):
        if ver == 'real_virtual':
            batch_carla = batch['data_carla']
            batch_nusc = batch['data_nusc']
            # Normalise from [0, 255] to [0, 1] 归一化
            # batch_carla['image'] = batch_carla['image'].float() / 255
            # batch_nusc['image'] = batch_nusc['image'].float() # nusc的图像已经在[0,1]范围了！！

            #没有route map，用的是command，前面已经处理好变成[1,2,3]选一个数
            ...
            batch_carla = functional_crop(batch_carla, self.crop)
            # if self.cfg.EVAL.RESOLUTION.ENABLED:
            #     batch = functional_resize_batch(batch, scale=1/self.cfg.EVAL.RESOLUTION.FACTOR)

            batch_carla = self.prepare_bev_labels(batch_carla)
            # batch_nusc = self.prepare_bev_labels(batch_nusc)
            # batch_carla = self.prepare_rgb_labels(batch_carla)
            # batch_nusc = self.prepare_rgb_labels(batch_nusc)

            if self.training:
                batch_carla = self.augmentation(batch_carla)
                batch_nusc = self.augmentation(batch_nusc)

            # Use imagenet mean and std normalisation, because we're loading pretrained backbones
            # 是否尺寸一致
            # batch_carla['image'] = (batch_carla['image'] - self.image_mean) / self.image_std
            # batch_nusc['image'] = (batch_nusc['image'] - self.image_mean) / self.image_std
            # if 'route_map' in batch:
            ...
            # future_xy为未来时刻每一步与当前时刻坐标的差值，是否需要归一化
            # batch_carla['future_xy'] = batch_carla['future_location'][:,:,:2]
            # batch_nusc['future_xy'] = batch_nusc['gt_trajectory'][:,1:,:2] - batch_nusc['gt_trajectory'][:,0,:2]

            return {'data_carla': batch_carla, 'data_nusc': batch_nusc}
...
# def inv_trans(img_list):
    ...
    # for i in img_list:
    #     i = i.squeeze(0)
    #     inv_image.append(inv_transform(i.cpu()).unsqueeze(0))
    # return inv_image
    
def denormalize(img_list, mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5)):
    """
    将归一化后的图像恢复到 [0, 1] 范围。
    输入: tensor 为 (C, H, W)，值在 [-1, 1] 之间
    """
    # print(img_list[0].shape)
    mean = torch.tensor(mean).view(1,-1, 1, 1).to(img_list[0].device)
    std = torch.tensor(std).view(1,-1, 1, 1).to(img_list[0].device)
    inv_image = []
    for tensor in img_list:
        inv_image.append(tensor * std + mean)
    return inv_image
...
class GANTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # self.dis_a.zero_grad()
        # self.dis_b.zero_grad()
        embedding_a = self.gen_a.gen_encoder(batch_a, batch_a['image'])
        embedding_b = self.gen_b.gen_encoder(batch_b, batch_b['image'])
        x_ba = self.gen_a.gen_decoder(embedding_b)
        x_ab = self.gen_b.gen_decoder(embedding_a)
        #loss_dis
        losses_dis = dict()
        ...
        embedding_b = self.gen_b.gen_encoder(batch_b, batch_b['image'])
        #within domain
        x_a_recon = self.gen_a.gen_decoder(embedding_a)
        x_b_recon = self.gen_b.gen_decoder(embedding_b)
        #cross domain
        x_ba = self.gen_a.gen_decoder(embedding_b)
        x_ab = self.gen_b.gen_decoder(embedding_a)
        #encode again
        embedding_ba = self.gen_a.gen_encoder(batch_b, x_ba['rgb_label_1'])
        embedding_ab = self.gen_b.gen_encoder(batch_a, x_ab['rgb_label_1'])
        #decode again
        x_aba = self.gen_a.gen_decoder(embedding_ab)
        x_bab = self.gen_b.gen_decoder(embedding_ba)

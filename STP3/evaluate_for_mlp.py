...
def evaluate(final_traj_path=None, checkpoint_path=None, dataroot=None, online=False):
    ...
    if final_traj_path is not None:
    ...
    if online == True:
        ...
        for index, batch in enumerate(tqdm(valloader)):
            ...
            for i in range(future_second):
                cur_time = (i+1)*2
                final_traj = torch.tensor([[[0.1880, 2.2542, 0.0000],
                         [0.8942, 4.8464, 0.0000],
                         [2.4961, 6.6214, 0.0000],
                         [3.7376, 8.4075, 0.0000],
                         [5.3880, 9.9690, 0.0000],
                         [7.5625, 11.2220, 0.0000]]], device='cuda:0')
                #final = torch.from_numpy(final_traj[batch["token"][0]])
                #final_traj[:, :cur_time]
                #labels['gt_trajectory'][:, 1:cur_time + 1]
                metric_planning_val[i](final_traj[:,:cur_time].to(device), labels['gt_trajectory'][:,1:cur_time+1].to(device), occupancy[:,:cur_time].to(device))
...
def run():
    ...
    if not online:
        ...
        evaluate(final_traj_path=None, checkpoint_path='ckpt/STP3_plan.ckpt', dataroot='/home/balink/nuscenes-devkit/data/sets/nuscenes',
                 online=True)

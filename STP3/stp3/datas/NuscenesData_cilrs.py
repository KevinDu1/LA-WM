...
class FuturePredictionDataset(torch.utils.data.Dataset):
    ...
    def get_gt_trajectory(self, rec, ref_index):
        ...
        n_output = 4
        gt_trajectory = np.zeros((n_output, 3), np.float64)
        ...
        return gt_trajectory, command_onehot
    ...
    def __getitem__(self, index):
        ...
        for i, index_t in enumerate(self.indices[index]):
            ...
            if i < self.receptive_field:
                data['img'],  data['calibrated']  = self.get_img(rec)
                '''
            ...
            if i == self.cfg.TIME_RECEPTIVE_FIELD-1:
                ...
                data['state'] = self.get_cilrs_state(rec, index_t, command)
                ...
                data['speed'] = self.get_speed(rec, index_t, data['gt_trajectory'])
                '''

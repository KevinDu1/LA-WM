    ...
    def get_speed(self, rec, sample_index, gt_traj=None, is_bus=True):
        ...
        if is_bus:
            ...
            if any(abs(veh_speed[:,0]-scene_time)<0.1):
                speed = veh_speed[np.argmin(abs(veh_speed[:,0]-scene_time)<0.1)][-1]
            else:
                speed_past = veh_speed[np.argmin(abs(veh_speed[:, 0] - scene_time + 0.5) < 0.1)][-1]
                speed_future = veh_speed[np.argmin(abs(veh_speed[:, 0] - scene_time - 0.5) < 0.1)][-1]

...
from data import ImageFilelist, ImageFolder_nusc, ImageFolder_carla
import torch
import os
import math
...
def get_all_data_loaders(conf):
    ...
    if 'data_root' in conf:
        train_loader_a = get_data_loader_folder_carla(os.path.join(conf['data_root'], 'trainA'), batch_size, True,
                                              new_size_a, height, width, num_workers, True)
        test_loader_a = get_data_loader_folder_carla(os.path.join(conf['data_root'], 'testA'), batch_size, False,
                                             new_size_a, new_size_a, new_size_a, num_workers, True)
        train_loader_b = get_data_loader_folder_nusc(os.path.join(conf['data_root'], 'trainB'), batch_size, True,
                                              new_size_b, height, width, num_workers, True)
        test_loader_b = get_data_loader_folder_nusc(os.path.join(conf['data_root'], 'testB'), batch_size, False,
...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
    ...
    transform_list = [transforms.RandomHorizontalFlip()] + transform_list if train else transform_list
    transform = transforms.Compose(transform_list)
    dataset = ImageFolder_nusc(input_folder, transform=transform)
    loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=train, drop_last=True, num_workers=num_workers)
    return loader

def get_data_loader_folder_nusc(input_folder, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=True):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.5, 0.5, 0.5),
                                           (0.5, 0.5, 0.5))]
...
def get_data_loader_folder_carla(input_folder, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=True):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.5, 0.5, 0.5),
                                           (0.5, 0.5, 0.5))]
    ...
    transform_list = [transforms.RandomHorizontalFlip()] + transform_list if train else transform_list
    transform = transforms.Compose(transform_list)
    dataset = ImageFolder_carla(input_folder, transform=transform)
    loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=train, drop_last=True, num_workers=num_workers)
    return loader
...
def __write_images_bev(image_outputs, display_image_num, file_name):
    image_outputs = [images.expand(-1, 3, -1, -1) for images in image_outputs] # expand gray-scale images to 3 channels
    image_tensor = torch.cat([images[:display_image_num] for images in image_outputs], 0)
...
def write_2images_test(image_outputs, display_image_num, image_directory, postfix):
    n = len(image_outputs)
    if not os.path.exists('%s/test'% image_directory):
        print("Creating directory: {}".format('%s/test'% image_directory))
        os.makedirs('%s/test'% image_directory)
    __write_images_bev(image_outputs[0:n//2], display_image_num, '%s/test/gen_%s_a2b.jpg' % (image_directory, postfix))
    __write_images_bev(image_outputs[n//2:n], display_image_num, '%s/test/gen_%s_b2a.jpg' % (image_directory, postfix))
